...
import { findAsymptoteDependencies, transformAsymptote } from "~/asymptote";
    ...
    configureServer(this, server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url === "/asy") {
          ...
          const body = (await json(req)) as { fileName: string; inject: object | null };
          let svg: { src: string; width: number; height: number };

          try {
            svg = await transformAsymptote(body.fileName, body.inject);
          } catch (err: any) {
            server.ws.send({
              type: "error",
              err: {
