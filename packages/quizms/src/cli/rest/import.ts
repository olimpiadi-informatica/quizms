...
import cas, { casRange } from "./utils/cas";
...
async function importContests(options: ImportOptions) {
  ...
  const res = await Promise.all(
    contests.map(async (contest) => {
      ...
      return await update(options, "contest", restContest);
    }),
  );
...
async function importParticipations(options: ImportOptions) {
  ...
  const res = await Promise.all(
    participations.map(async (participation) => {
      ...
      return await update(options, "venue", venue);
    }),
  );
  const total = res.reduce<number>((acc, val) => acc + val, 0);
...
async function importStudents(options: ImportOptions) {
  ...
  const res = await Promise.all(
    students.map(async (student) => {
      ...
      const added = await update(options, "student_data", restStudent);
  ...
  }
  {
    ...
    const res = await Promise.all([
      ...restVariants.map(async (variant) => {
        return await update(options, "variant", variant);
      }),
    ]);
    const total = res.reduce<number>((acc, val) => acc + val, 0);
  ...
  }
  {
    ...
    const res = await Promise.all([
      ...Object.values(problems).map(async (problem) => {
        return await update(options, "problem", problem);
      }),
    ]);
    const total = res.reduce<number>((acc, val) => acc + val, 0);
...
async function update(option: ImportOptions, collection: string, newVal: any) {
  if (option.verbose) {
  ...
  return await cas(option, body, collection);
}
